...
const buildCabinetRun = ({ parent, length, x = 0, z = 0, rotation = 0, front, upperMode, primary = false }) => {
  ...
  const useWood = front.texture === 'wood' || front.label.includes('шпон') || front.label.includes('массив');
...
const addIsland = (parent, front) => {
  const useWood = front.texture === 'wood';
  const islandMap = useWood ? createWoodTexture(front.color) : null;
  const islandMaterials = {
    ...
    counter: new THREE.MeshStandardMaterial({ color: '#f6f2e8', map: createStoneTexture(), roughness: 0.32 }),
    plinth: new THREE.MeshStandardMaterial({ color: '#1d201f', roughness: 0.44, metalness: 0.1 }),
  };
  ...
  parent.add(island);
  addBox(island, [1.55, 0.74, 0.78], [0, 0.43, 0], islandMaterials.body, true);
  addBox(island, [1.7, 0.08, 0.92], [0, 0.84, 0], islandMaterials.counter);
  addBox(island, [1.36, 0.08, 0.08], [0, 0.08, -0.32], islandMaterials.plinth);
  addBox(island, [0.74, 0.018, 0.026], [0, 1.04, 0.48], islandMaterials.counter);
};
